...
# ── exploration journal (extracted to exploration.py) ─────────────────────────
# The feature's functions live in ``exploration``; its state/constants above stay
# here so the module namespace tests monkeypatch is unchanged. The module resolves
# all shared state/services and its own siblings through the host we bind here, so
# ``lp_web.<name>`` (and host-level monkeypatches of it) keep working. We bind a
# thin proxy over this module's globals() rather than the module object, because
# the test-suite loads lp-web.py under several names (and via module_from_spec,
# which never registers it in sys.modules) — but globals() is always this exact
# namespace, and ``setattr(module, …)`` writes straight into it.
class _HostProxy:
    """Attribute access that reads this module's live globals, so every lookup
    (and every monkeypatch of a host attribute) resolves against one namespace."""
    def __init__(self, g):
...
import exploration as _exploration
_EXPLORATION = _exploration.bind(_HostProxy(globals()))
for _name in _exploration.EXPORTS:
    globals()[_name] = getattr(_EXPLORATION, _name)
del _name
